<%@ page language="java" contentType="text/html; charset=ISO-8859-1"
	pageEncoding="ISO-8859-1"%>
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core"%>

<%@ taglib prefix="ct" uri="/WEB-INF/CustomTag.tld"%>
<!DOCTYPE html>
<html>
<head>
<meta http-equiv="Content-Type" content="text/html; charset=ISO-8859-1">
<title>Order List</title>
<jsp:include page="basic.jsp"></jsp:include>
</head>
<body>
	<div id="shoppingCart">
		<div class="orderList">
			<h1>In My Cart</h1>
			<c:forEach items="${sessionScope['cart'].products}" var="product">
				<div class="goodCheckout">
					<a><img alt="No image" src="<c:url value="resources/image/${product.firstImage}"/>"></a>
					<h3>${product.productName}</h3>
					<p>$${product.price}</p>
					<a class="delete" href="<c:url value='orderList?action=delete&productId=${product.productId}'/>">Delete</a>
				</div>
			</c:forEach>
		</div>
		<div class="total">Sub total (${sessionScope['cart'].size} item(s)): <ct:span fontWeight="700" color="#676767" fontSize="20px !important" text="${sessionScope['cart'].money}$"/></div>
		<div>
			<c:if test="${not empty sessionScope['cart'].products}">
					<a href="checkout" id="checkoutBtn">Checkout</a>
			</c:if>
		</div>
	</div>
</body>
</html>